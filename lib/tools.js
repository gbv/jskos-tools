 ...
 *
 * @param {*} jskos object or array of objects
 */
...
const mappingToCSV = (options = {}) => {
  ...
  return mapping => {
    let from = getNested(mapping, "from.memberSet.0.notation.0")
    let to = getNested(mapping, "to.memberSet.0.notation.0")
    to = to !== null ? to : "" // support empty mapping target
    let type = mappingTypeByUri(getNested(mapping, "type.0"))
    if (from !== null && to !== null) {
      let fields = language ? [
        from,
        getNested(mapping, `from.memberSet.0.prefLabel.${language}`),
        to,
        getNested(mapping, `to.memberSet.0.prefLabel.${language}`)
      ] : [ from, to ]
      fields.push(type ? type.short : "")
...
module.exports = {
  addContext, clean, cleanJSKOS, copyDeep, deepCopy, getAllUris, compare,compareObjects, compareSchemes, compareConcepts, isConcept, isScheme, isContainedIn, isSchemeInList, sortConcepts, sortSchemes, minifyMapping, mappingTypes, mappingTypeByUri, mappingTypeByType, mappingToCSV, defaultMappingType, conceptsOfMapping,
}
