window.lunrData = {
  "index": {
    "version": "1.0.0",
    "fields": [
      {
        "name": "longname",
        "boost": 1000
      },
      {
        "name": "name",
        "boost": 500
      },
      {
        "name": "tags",
        "boost": 300
      },
      {
        "name": "kind",
        "boost": 110
      },
      {
        "name": "title",
        "boost": 100
      },
      {
        "name": "summary",
        "boost": 70
      },
      {
        "name": "description",
        "boost": 50
      },
      {
        "name": "body",
        "boost": 1
      }
    ],
    "ref": "id",
    "tokenizer": "default",
    "documentStore": {
      "store": {
        "index.html": [
          "data",
          "format",
          "index",
          "jsko",
          "readm",
          "tool",
          "work"
        ],
        "global.html": [
          "document",
          "global"
        ],
        "list_class.html": [
          "class",
          "document",
          "list",
          "list:class"
        ],
        "list_module.html": [
          "document",
          "list",
          "list:modul",
          "modul"
        ],
        "module-jskos-tools.ConceptScheme.html": [
          "class",
          "concept",
          "conceptschem",
          "jsko",
          "module:jsko",
          "scheme",
          "tools.conceptschem"
        ],
        "module-jskos-tools.ConceptScheme.html#isValidNotation": [
          "array|nul",
          "check",
          "conceptscheme#isvalidnot",
          "defin",
          "function",
          "isvalidnot",
          "module:jsko",
          "notat",
          "notationpattern",
          "string",
          "tools.conceptscheme#isvalidnot",
          "valid",
          "whether"
        ],
        "module-jskos-tools.ConceptScheme.html#notationFromUri": [
          "belong",
          "check",
          "conceptscheme#notationfromuri",
          "function",
          "local",
          "module:jsko",
          "namespac",
          "notat",
          "notationfromuri",
          "requir",
          "return",
          "scheme",
          "string|undefin",
          "success",
          "tools.conceptscheme#notationfromuri",
          "uri",
          "uripattern",
          "whether"
        ],
        "module-jskos-tools.ConceptScheme.html#uriFromNotation": [
          "check",
          "conceptscheme#urifromnot",
          "function",
          "local",
          "map",
          "module:jsko",
          "namespac",
          "notat",
          "requir",
          "scheme",
          "tools.conceptscheme#urifromnot",
          "uri",
          "urifromnot",
          "uripattern",
          "valid",
          "whether"
        ],
        "module-jskos-tools.ConceptScheme.html#conceptFromUri": [
          "belong",
          "check",
          "concept",
          "conceptfromuri",
          "conceptscheme#conceptfromuri",
          "function",
          "module:jsko",
          "namespac",
          "option",
          "requir",
          "return",
          "scheme",
          "success",
          "tools.conceptscheme#conceptfromuri",
          "uri",
          "uripattern",
          "whether"
        ],
        "module-jskos-tools.ConceptScheme.html#conceptFromNotation": [
          "concept",
          "conceptfromnot",
          "conceptscheme#conceptfromnot",
          "function",
          "local",
          "map",
          "module:jsko",
          "namespac",
          "notat",
          "option",
          "requir",
          "scheme",
          "tools.conceptscheme#conceptfromnot",
          "uripattern",
          "valid"
        ],
        "module-jskos-tools.html": [
          "jsko",
          "modul",
          "module:jsko",
          "tool"
        ]
      },
      "length": 11
    },
    "tokenStore": {
      "root": {
        "docs": {},
        "d": {
          "docs": {},
          "a": {
            "docs": {},
            "t": {
              "docs": {},
              "a": {
                "docs": {
                  "index.html": {
                    "ref": "index.html",
                    "tf": 14
                  }
                }
              }
            }
          },
          "o": {
            "docs": {},
            "c": {
              "docs": {},
              "u": {
                "docs": {},
                "m": {
                  "docs": {},
                  "e": {
                    "docs": {},
                    "n": {
                      "docs": {},
                      "t": {
                        "docs": {
                          "global.html": {
                            "ref": "global.html",
                            "tf": 35
                          },
                          "list_class.html": {
                            "ref": "list_class.html",
                            "tf": 35
                          },
                          "list_module.html": {
                            "ref": "list_module.html",
                            "tf": 35
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "e": {
            "docs": {},
            "f": {
              "docs": {},
              "i": {
                "docs": {},
                "n": {
                  "docs": {
                    "module-jskos-tools.ConceptScheme.html#isValidNotation": {
                      "ref": "module-jskos-tools.ConceptScheme.html#isValidNotation",
                      "tf": 7.142857142857142
                    }
                  }
                }
              }
            }
          }
        },
        "f": {
          "docs": {},
          "o": {
            "docs": {},
            "r": {
              "docs": {},
              "m": {
                "docs": {},
                "a": {
                  "docs": {},
                  "t": {
                    "docs": {
                      "index.html": {
                        "ref": "index.html",
                        "tf": 14
                      }
                    }
                  }
                }
              }
            }
          },
          "u": {
            "docs": {},
            "n": {
              "docs": {},
              "c": {
                "docs": {},
                "t": {
                  "docs": {},
                  "i": {
                    "docs": {},
                    "o": {
                      "docs": {},
                      "n": {
                        "docs": {
                          "module-jskos-tools.ConceptScheme.html#isValidNotation": {
                            "ref": "module-jskos-tools.ConceptScheme.html#isValidNotation",
                            "tf": 110
                          },
                          "module-jskos-tools.ConceptScheme.html#notationFromUri": {
                            "ref": "module-jskos-tools.ConceptScheme.html#notationFromUri",
                            "tf": 110
                          },
                          "module-jskos-tools.ConceptScheme.html#uriFromNotation": {
                            "ref": "module-jskos-tools.ConceptScheme.html#uriFromNotation",
                            "tf": 110
                          },
                          "module-jskos-tools.ConceptScheme.html#conceptFromUri": {
                            "ref": "module-jskos-tools.ConceptScheme.html#conceptFromUri",
                            "tf": 110
                          },
                          "module-jskos-tools.ConceptScheme.html#conceptFromNotation": {
                            "ref": "module-jskos-tools.ConceptScheme.html#conceptFromNotation",
                            "tf": 110
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "i": {
          "docs": {},
          "n": {
            "docs": {},
            "d": {
              "docs": {},
              "e": {
                "docs": {},
                "x": {
                  "docs": {
                    "index.html": {
                      "ref": "index.html",
                      "tf": 1300
                    }
                  }
                }
              }
            }
          },
          "s": {
            "docs": {},
            "v": {
              "docs": {},
              "a": {
                "docs": {},
                "l": {
                  "docs": {},
                  "i": {
                    "docs": {},
                    "d": {
                      "docs": {},
                      "n": {
                        "docs": {},
                        "o": {
                          "docs": {},
                          "t": {
                            "docs": {
                              "module-jskos-tools.ConceptScheme.html#isValidNotation": {
                                "ref": "module-jskos-tools.ConceptScheme.html#isValidNotation",
                                "tf": 608.3333333333334
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "j": {
          "docs": {},
          "s": {
            "docs": {},
            "k": {
              "docs": {},
              "o": {
                "docs": {
                  "index.html": {
                    "ref": "index.html",
                    "tf": 314
                  },
                  "module-jskos-tools.ConceptScheme.html": {
                    "ref": "module-jskos-tools.ConceptScheme.html",
                    "tf": 66.66666666666666
                  },
                  "module-jskos-tools.html": {
                    "ref": "module-jskos-tools.html",
                    "tf": 325
                  }
                }
              }
            }
          }
        },
        "r": {
          "docs": {},
          "e": {
            "docs": {},
            "a": {
              "docs": {},
              "d": {
                "docs": {},
                "m": {
                  "docs": {
                    "index.html": {
                      "ref": "index.html",
                      "tf": 110
                    }
                  }
                }
              }
            },
            "q": {
              "docs": {},
              "u": {
                "docs": {},
                "i": {
                  "docs": {},
                  "r": {
                    "docs": {
                      "module-jskos-tools.ConceptScheme.html#notationFromUri": {
                        "ref": "module-jskos-tools.ConceptScheme.html#notationFromUri",
                        "tf": 3.8461538461538463
                      },
                      "module-jskos-tools.ConceptScheme.html#uriFromNotation": {
                        "ref": "module-jskos-tools.ConceptScheme.html#uriFromNotation",
                        "tf": 4.166666666666666
                      },
                      "module-jskos-tools.ConceptScheme.html#conceptFromUri": {
                        "ref": "module-jskos-tools.ConceptScheme.html#conceptFromUri",
                        "tf": 4.166666666666666
                      },
                      "module-jskos-tools.ConceptScheme.html#conceptFromNotation": {
                        "ref": "module-jskos-tools.ConceptScheme.html#conceptFromNotation",
                        "tf": 5
                      }
                    }
                  }
                }
              }
            },
            "t": {
              "docs": {},
              "u": {
                "docs": {},
                "r": {
                  "docs": {},
                  "n": {
                    "docs": {
                      "module-jskos-tools.ConceptScheme.html#notationFromUri": {
                        "ref": "module-jskos-tools.ConceptScheme.html#notationFromUri",
                        "tf": 3.8461538461538463
                      },
                      "module-jskos-tools.ConceptScheme.html#conceptFromUri": {
                        "ref": "module-jskos-tools.ConceptScheme.html#conceptFromUri",
                        "tf": 4.166666666666666
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "t": {
          "docs": {},
          "o": {
            "docs": {},
            "o": {
              "docs": {},
              "l": {
                "docs": {
                  "index.html": {
                    "ref": "index.html",
                    "tf": 314
                  },
                  "module-jskos-tools.html": {
                    "ref": "module-jskos-tools.html",
                    "tf": 975
                  }
                },
                "s": {
                  "docs": {},
                  ".": {
                    "docs": {},
                    "c": {
                      "docs": {},
                      "o": {
                        "docs": {},
                        "n": {
                          "docs": {},
                          "c": {
                            "docs": {},
                            "e": {
                              "docs": {},
                              "p": {
                                "docs": {},
                                "t": {
                                  "docs": {},
                                  "s": {
                                    "docs": {},
                                    "c": {
                                      "docs": {},
                                      "h": {
                                        "docs": {},
                                        "e": {
                                          "docs": {},
                                          "m": {
                                            "docs": {
                                              "module-jskos-tools.ConceptScheme.html": {
                                                "ref": "module-jskos-tools.ConceptScheme.html",
                                                "tf": 650
                                              }
                                            },
                                            "e": {
                                              "docs": {},
                                              "#": {
                                                "docs": {},
                                                "i": {
                                                  "docs": {},
                                                  "s": {
                                                    "docs": {},
                                                    "v": {
                                                      "docs": {},
                                                      "a": {
                                                        "docs": {},
                                                        "l": {
                                                          "docs": {},
                                                          "i": {
                                                            "docs": {},
                                                            "d": {
                                                              "docs": {},
                                                              "n": {
                                                                "docs": {},
                                                                "o": {
                                                                  "docs": {},
                                                                  "t": {
                                                                    "docs": {
                                                                      "module-jskos-tools.ConceptScheme.html#isValidNotation": {
                                                                        "ref": "module-jskos-tools.ConceptScheme.html#isValidNotation",
                                                                        "tf": 575
                                                                      }
                                                                    }
                                                                  }
                                                                }
                                                              }
                                                            }
                                                          }
                                                        }
                                                      }
                                                    }
                                                  }
                                                },
                                                "n": {
                                                  "docs": {},
                                                  "o": {
                                                    "docs": {},
                                                    "t": {
                                                      "docs": {},
                                                      "a": {
                                                        "docs": {},
                                                        "t": {
                                                          "docs": {},
                                                          "i": {
                                                            "docs": {},
                                                            "o": {
                                                              "docs": {},
                                                              "n": {
                                                                "docs": {},
                                                                "f": {
                                                                  "docs": {},
                                                                  "r": {
                                                                    "docs": {},
                                                                    "o": {
                                                                      "docs": {},
                                                                      "m": {
                                                                        "docs": {},
                                                                        "u": {
                                                                          "docs": {},
                                                                          "r": {
                                                                            "docs": {},
                                                                            "i": {
                                                                              "docs": {
                                                                                "module-jskos-tools.ConceptScheme.html#notationFromUri": {
                                                                                  "ref": "module-jskos-tools.ConceptScheme.html#notationFromUri",
                                                                                  "tf": 575
                                                                                }
                                                                              }
                                                                            }
                                                                          }
                                                                        }
                                                                      }
                                                                    }
                                                                  }
                                                                }
                                                              }
                                                            }
                                                          }
                                                        }
                                                      }
                                                    }
                                                  }
                                                },
                                                "u": {
                                                  "docs": {},
                                                  "r": {
                                                    "docs": {},
                                                    "i": {
                                                      "docs": {},
                                                      "f": {
                                                        "docs": {},
                                                        "r": {
                                                          "docs": {},
                                                          "o": {
                                                            "docs": {},
                                                            "m": {
                                                              "docs": {},
                                                              "n": {
                                                                "docs": {},
                                                                "o": {
                                                                  "docs": {},
                                                                  "t": {
                                                                    "docs": {
                                                                      "module-jskos-tools.ConceptScheme.html#uriFromNotation": {
                                                                        "ref": "module-jskos-tools.ConceptScheme.html#uriFromNotation",
                                                                        "tf": 575
                                                                      }
                                                                    }
                                                                  }
                                                                }
                                                              }
                                                            }
                                                          }
                                                        }
                                                      }
                                                    }
                                                  }
                                                },
                                                "c": {
                                                  "docs": {},
                                                  "o": {
                                                    "docs": {},
                                                    "n": {
                                                      "docs": {},
                                                      "c": {
                                                        "docs": {},
                                                        "e": {
                                                          "docs": {},
                                                          "p": {
                                                            "docs": {},
                                                            "t": {
                                                              "docs": {},
                                                              "f": {
                                                                "docs": {},
                                                                "r": {
                                                                  "docs": {},
                                                                  "o": {
                                                                    "docs": {},
                                                                    "m": {
                                                                      "docs": {},
                                                                      "u": {
                                                                        "docs": {},
                                                                        "r": {
                                                                          "docs": {},
                                                                          "i": {
                                                                            "docs": {
                                                                              "module-jskos-tools.ConceptScheme.html#conceptFromUri": {
                                                                                "ref": "module-jskos-tools.ConceptScheme.html#conceptFromUri",
                                                                                "tf": 575
                                                                              }
                                                                            }
                                                                          }
                                                                        }
                                                                      },
                                                                      "n": {
                                                                        "docs": {},
                                                                        "o": {
                                                                          "docs": {},
                                                                          "t": {
                                                                            "docs": {
                                                                              "module-jskos-tools.ConceptScheme.html#conceptFromNotation": {
                                                                                "ref": "module-jskos-tools.ConceptScheme.html#conceptFromNotation",
                                                                                "tf": 575
                                                                              }
                                                                            }
                                                                          }
                                                                        }
                                                                      }
                                                                    }
                                                                  }
                                                                }
                                                              }
                                                            }
                                                          }
                                                        }
                                                      }
                                                    }
                                                  }
                                                }
                                              }
                                            }
                                          }
                                        }
                                      }
                                    }
                                  }
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "w": {
          "docs": {},
          "o": {
            "docs": {},
            "r": {
              "docs": {},
              "k": {
                "docs": {
                  "index.html": {
                    "ref": "index.html",
                    "tf": 14
                  }
                }
              }
            }
          },
          "h": {
            "docs": {},
            "e": {
              "docs": {},
              "t": {
                "docs": {},
                "h": {
                  "docs": {},
                  "e": {
                    "docs": {},
                    "r": {
                      "docs": {
                        "module-jskos-tools.ConceptScheme.html#isValidNotation": {
                          "ref": "module-jskos-tools.ConceptScheme.html#isValidNotation",
                          "tf": 7.142857142857142
                        },
                        "module-jskos-tools.ConceptScheme.html#notationFromUri": {
                          "ref": "module-jskos-tools.ConceptScheme.html#notationFromUri",
                          "tf": 3.8461538461538463
                        },
                        "module-jskos-tools.ConceptScheme.html#uriFromNotation": {
                          "ref": "module-jskos-tools.ConceptScheme.html#uriFromNotation",
                          "tf": 4.166666666666666
                        },
                        "module-jskos-tools.ConceptScheme.html#conceptFromUri": {
                          "ref": "module-jskos-tools.ConceptScheme.html#conceptFromUri",
                          "tf": 4.166666666666666
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "g": {
          "docs": {},
          "l": {
            "docs": {},
            "o": {
              "docs": {},
              "b": {
                "docs": {},
                "a": {
                  "docs": {},
                  "l": {
                    "docs": {
                      "global.html": {
                        "ref": "global.html",
                        "tf": 2045
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "c": {
          "docs": {},
          "l": {
            "docs": {},
            "a": {
              "docs": {},
              "s": {
                "docs": {},
                "s": {
                  "docs": {
                    "list_class.html": {
                      "ref": "list_class.html",
                      "tf": 635
                    },
                    "module-jskos-tools.ConceptScheme.html": {
                      "ref": "module-jskos-tools.ConceptScheme.html",
                      "tf": 110
                    }
                  }
                }
              }
            }
          },
          "o": {
            "docs": {},
            "n": {
              "docs": {},
              "c": {
                "docs": {},
                "e": {
                  "docs": {},
                  "p": {
                    "docs": {},
                    "t": {
                      "docs": {
                        "module-jskos-tools.ConceptScheme.html": {
                          "ref": "module-jskos-tools.ConceptScheme.html",
                          "tf": 16.666666666666664
                        },
                        "module-jskos-tools.ConceptScheme.html#conceptFromUri": {
                          "ref": "module-jskos-tools.ConceptScheme.html#conceptFromUri",
                          "tf": 4.166666666666666
                        },
                        "module-jskos-tools.ConceptScheme.html#conceptFromNotation": {
                          "ref": "module-jskos-tools.ConceptScheme.html#conceptFromNotation",
                          "tf": 5
                        }
                      },
                      "s": {
                        "docs": {},
                        "c": {
                          "docs": {},
                          "h": {
                            "docs": {},
                            "e": {
                              "docs": {},
                              "m": {
                                "docs": {
                                  "module-jskos-tools.ConceptScheme.html": {
                                    "ref": "module-jskos-tools.ConceptScheme.html",
                                    "tf": 600
                                  }
                                },
                                "e": {
                                  "docs": {},
                                  "#": {
                                    "docs": {},
                                    "i": {
                                      "docs": {},
                                      "s": {
                                        "docs": {},
                                        "v": {
                                          "docs": {},
                                          "a": {
                                            "docs": {},
                                            "l": {
                                              "docs": {},
                                              "i": {
                                                "docs": {},
                                                "d": {
                                                  "docs": {},
                                                  "n": {
                                                    "docs": {},
                                                    "o": {
                                                      "docs": {},
                                                      "t": {
                                                        "docs": {
                                                          "module-jskos-tools.ConceptScheme.html#isValidNotation": {
                                                            "ref": "module-jskos-tools.ConceptScheme.html#isValidNotation",
                                                            "tf": 75
                                                          }
                                                        }
                                                      }
                                                    }
                                                  }
                                                }
                                              }
                                            }
                                          }
                                        }
                                      }
                                    },
                                    "n": {
                                      "docs": {},
                                      "o": {
                                        "docs": {},
                                        "t": {
                                          "docs": {},
                                          "a": {
                                            "docs": {},
                                            "t": {
                                              "docs": {},
                                              "i": {
                                                "docs": {},
                                                "o": {
                                                  "docs": {},
                                                  "n": {
                                                    "docs": {},
                                                    "f": {
                                                      "docs": {},
                                                      "r": {
                                                        "docs": {},
                                                        "o": {
                                                          "docs": {},
                                                          "m": {
                                                            "docs": {},
                                                            "u": {
                                                              "docs": {},
                                                              "r": {
                                                                "docs": {},
                                                                "i": {
                                                                  "docs": {
                                                                    "module-jskos-tools.ConceptScheme.html#notationFromUri": {
                                                                      "ref": "module-jskos-tools.ConceptScheme.html#notationFromUri",
                                                                      "tf": 75
                                                                    }
                                                                  }
                                                                }
                                                              }
                                                            }
                                                          }
                                                        }
                                                      }
                                                    }
                                                  }
                                                }
                                              }
                                            }
                                          }
                                        }
                                      }
                                    },
                                    "u": {
                                      "docs": {},
                                      "r": {
                                        "docs": {},
                                        "i": {
                                          "docs": {},
                                          "f": {
                                            "docs": {},
                                            "r": {
                                              "docs": {},
                                              "o": {
                                                "docs": {},
                                                "m": {
                                                  "docs": {},
                                                  "n": {
                                                    "docs": {},
                                                    "o": {
                                                      "docs": {},
                                                      "t": {
                                                        "docs": {
                                                          "module-jskos-tools.ConceptScheme.html#uriFromNotation": {
                                                            "ref": "module-jskos-tools.ConceptScheme.html#uriFromNotation",
                                                            "tf": 75
                                                          }
                                                        }
                                                      }
                                                    }
                                                  }
                                                }
                                              }
                                            }
                                          }
                                        }
                                      }
                                    },
                                    "c": {
                                      "docs": {},
                                      "o": {
                                        "docs": {},
                                        "n": {
                                          "docs": {},
                                          "c": {
                                            "docs": {},
                                            "e": {
                                              "docs": {},
                                              "p": {
                                                "docs": {},
                                                "t": {
                                                  "docs": {},
                                                  "f": {
                                                    "docs": {},
                                                    "r": {
                                                      "docs": {},
                                                      "o": {
                                                        "docs": {},
                                                        "m": {
                                                          "docs": {},
                                                          "u": {
                                                            "docs": {},
                                                            "r": {
                                                              "docs": {},
                                                              "i": {
                                                                "docs": {
                                                                  "module-jskos-tools.ConceptScheme.html#conceptFromUri": {
                                                                    "ref": "module-jskos-tools.ConceptScheme.html#conceptFromUri",
                                                                    "tf": 75
                                                                  }
                                                                }
                                                              }
                                                            }
                                                          },
                                                          "n": {
                                                            "docs": {},
                                                            "o": {
                                                              "docs": {},
                                                              "t": {
                                                                "docs": {
                                                                  "module-jskos-tools.ConceptScheme.html#conceptFromNotation": {
                                                                    "ref": "module-jskos-tools.ConceptScheme.html#conceptFromNotation",
                                                                    "tf": 75
                                                                  }
                                                                }
                                                              }
                                                            }
                                                          }
                                                        }
                                                      }
                                                    }
                                                  }
                                                }
                                              }
                                            }
                                          }
                                        }
                                      }
                                    }
                                  }
                                }
                              }
                            }
                          }
                        }
                      },
                      "f": {
                        "docs": {},
                        "r": {
                          "docs": {},
                          "o": {
                            "docs": {},
                            "m": {
                              "docs": {},
                              "u": {
                                "docs": {},
                                "r": {
                                  "docs": {},
                                  "i": {
                                    "docs": {
                                      "module-jskos-tools.ConceptScheme.html#conceptFromUri": {
                                        "ref": "module-jskos-tools.ConceptScheme.html#conceptFromUri",
                                        "tf": 608.3333333333334
                                      }
                                    }
                                  }
                                }
                              },
                              "n": {
                                "docs": {},
                                "o": {
                                  "docs": {},
                                  "t": {
                                    "docs": {
                                      "module-jskos-tools.ConceptScheme.html#conceptFromNotation": {
                                        "ref": "module-jskos-tools.ConceptScheme.html#conceptFromNotation",
                                        "tf": 608.3333333333334
                                      }
                                    }
                                  }
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "h": {
            "docs": {},
            "e": {
              "docs": {},
              "c": {
                "docs": {},
                "k": {
                  "docs": {
                    "module-jskos-tools.ConceptScheme.html#isValidNotation": {
                      "ref": "module-jskos-tools.ConceptScheme.html#isValidNotation",
                      "tf": 7.142857142857142
                    },
                    "module-jskos-tools.ConceptScheme.html#notationFromUri": {
                      "ref": "module-jskos-tools.ConceptScheme.html#notationFromUri",
                      "tf": 3.8461538461538463
                    },
                    "module-jskos-tools.ConceptScheme.html#uriFromNotation": {
                      "ref": "module-jskos-tools.ConceptScheme.html#uriFromNotation",
                      "tf": 4.166666666666666
                    },
                    "module-jskos-tools.ConceptScheme.html#conceptFromUri": {
                      "ref": "module-jskos-tools.ConceptScheme.html#conceptFromUri",
                      "tf": 4.166666666666666
                    }
                  }
                }
              }
            }
          }
        },
        "l": {
          "docs": {},
          "i": {
            "docs": {},
            "s": {
              "docs": {},
              "t": {
                "docs": {
                  "list_class.html": {
                    "ref": "list_class.html",
                    "tf": 110
                  },
                  "list_module.html": {
                    "ref": "list_module.html",
                    "tf": 110
                  }
                },
                ":": {
                  "docs": {},
                  "c": {
                    "docs": {},
                    "l": {
                      "docs": {},
                      "a": {
                        "docs": {},
                        "s": {
                          "docs": {},
                          "s": {
                            "docs": {
                              "list_class.html": {
                                "ref": "list_class.html",
                                "tf": 1300
                              }
                            }
                          }
                        }
                      }
                    }
                  },
                  "m": {
                    "docs": {},
                    "o": {
                      "docs": {},
                      "d": {
                        "docs": {},
                        "u": {
                          "docs": {},
                          "l": {
                            "docs": {
                              "list_module.html": {
                                "ref": "list_module.html",
                                "tf": 1300
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "o": {
            "docs": {},
            "c": {
              "docs": {},
              "a": {
                "docs": {},
                "l": {
                  "docs": {
                    "module-jskos-tools.ConceptScheme.html#notationFromUri": {
                      "ref": "module-jskos-tools.ConceptScheme.html#notationFromUri",
                      "tf": 3.8461538461538463
                    },
                    "module-jskos-tools.ConceptScheme.html#uriFromNotation": {
                      "ref": "module-jskos-tools.ConceptScheme.html#uriFromNotation",
                      "tf": 4.166666666666666
                    },
                    "module-jskos-tools.ConceptScheme.html#conceptFromNotation": {
                      "ref": "module-jskos-tools.ConceptScheme.html#conceptFromNotation",
                      "tf": 5
                    }
                  }
                }
              }
            }
          }
        },
        "m": {
          "docs": {},
          "o": {
            "docs": {},
            "d": {
              "docs": {},
              "u": {
                "docs": {},
                "l": {
                  "docs": {
                    "list_module.html": {
                      "ref": "list_module.html",
                      "tf": 635
                    },
                    "module-jskos-tools.html": {
                      "ref": "module-jskos-tools.html",
                      "tf": 110
                    }
                  },
                  "e": {
                    "docs": {},
                    ":": {
                      "docs": {},
                      "j": {
                        "docs": {},
                        "s": {
                          "docs": {},
                          "k": {
                            "docs": {},
                            "o": {
                              "docs": {
                                "module-jskos-tools.ConceptScheme.html": {
                                  "ref": "module-jskos-tools.ConceptScheme.html",
                                  "tf": 600
                                },
                                "module-jskos-tools.ConceptScheme.html#isValidNotation": {
                                  "ref": "module-jskos-tools.ConceptScheme.html#isValidNotation",
                                  "tf": 575
                                },
                                "module-jskos-tools.ConceptScheme.html#notationFromUri": {
                                  "ref": "module-jskos-tools.ConceptScheme.html#notationFromUri",
                                  "tf": 575
                                },
                                "module-jskos-tools.ConceptScheme.html#uriFromNotation": {
                                  "ref": "module-jskos-tools.ConceptScheme.html#uriFromNotation",
                                  "tf": 575
                                },
                                "module-jskos-tools.ConceptScheme.html#conceptFromUri": {
                                  "ref": "module-jskos-tools.ConceptScheme.html#conceptFromUri",
                                  "tf": 575
                                },
                                "module-jskos-tools.ConceptScheme.html#conceptFromNotation": {
                                  "ref": "module-jskos-tools.ConceptScheme.html#conceptFromNotation",
                                  "tf": 575
                                },
                                "module-jskos-tools.html": {
                                  "ref": "module-jskos-tools.html",
                                  "tf": 650
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "a": {
            "docs": {},
            "p": {
              "docs": {
                "module-jskos-tools.ConceptScheme.html#uriFromNotation": {
                  "ref": "module-jskos-tools.ConceptScheme.html#uriFromNotation",
                  "tf": 4.166666666666666
                },
                "module-jskos-tools.ConceptScheme.html#conceptFromNotation": {
                  "ref": "module-jskos-tools.ConceptScheme.html#conceptFromNotation",
                  "tf": 5
                }
              }
            }
          }
        },
        "s": {
          "docs": {},
          "c": {
            "docs": {},
            "h": {
              "docs": {},
              "e": {
                "docs": {},
                "m": {
                  "docs": {},
                  "e": {
                    "docs": {
                      "module-jskos-tools.ConceptScheme.html": {
                        "ref": "module-jskos-tools.ConceptScheme.html",
                        "tf": 16.666666666666664
                      },
                      "module-jskos-tools.ConceptScheme.html#notationFromUri": {
                        "ref": "module-jskos-tools.ConceptScheme.html#notationFromUri",
                        "tf": 7.6923076923076925
                      },
                      "module-jskos-tools.ConceptScheme.html#uriFromNotation": {
                        "ref": "module-jskos-tools.ConceptScheme.html#uriFromNotation",
                        "tf": 4.166666666666666
                      },
                      "module-jskos-tools.ConceptScheme.html#conceptFromUri": {
                        "ref": "module-jskos-tools.ConceptScheme.html#conceptFromUri",
                        "tf": 8.333333333333332
                      },
                      "module-jskos-tools.ConceptScheme.html#conceptFromNotation": {
                        "ref": "module-jskos-tools.ConceptScheme.html#conceptFromNotation",
                        "tf": 5
                      }
                    }
                  }
                }
              }
            }
          },
          "t": {
            "docs": {},
            "r": {
              "docs": {},
              "i": {
                "docs": {},
                "n": {
                  "docs": {},
                  "g": {
                    "docs": {
                      "module-jskos-tools.ConceptScheme.html#isValidNotation": {
                        "ref": "module-jskos-tools.ConceptScheme.html#isValidNotation",
                        "tf": 7.142857142857142
                      }
                    },
                    "|": {
                      "docs": {},
                      "u": {
                        "docs": {},
                        "n": {
                          "docs": {},
                          "d": {
                            "docs": {},
                            "e": {
                              "docs": {},
                              "f": {
                                "docs": {},
                                "i": {
                                  "docs": {},
                                  "n": {
                                    "docs": {
                                      "module-jskos-tools.ConceptScheme.html#notationFromUri": {
                                        "ref": "module-jskos-tools.ConceptScheme.html#notationFromUri",
                                        "tf": 33.33333333333333
                                      }
                                    }
                                  }
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "u": {
            "docs": {},
            "c": {
              "docs": {},
              "c": {
                "docs": {},
                "e": {
                  "docs": {},
                  "s": {
                    "docs": {},
                    "s": {
                      "docs": {
                        "module-jskos-tools.ConceptScheme.html#notationFromUri": {
                          "ref": "module-jskos-tools.ConceptScheme.html#notationFromUri",
                          "tf": 3.8461538461538463
                        },
                        "module-jskos-tools.ConceptScheme.html#conceptFromUri": {
                          "ref": "module-jskos-tools.ConceptScheme.html#conceptFromUri",
                          "tf": 4.166666666666666
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "a": {
          "docs": {},
          "r": {
            "docs": {},
            "r": {
              "docs": {},
              "a": {
                "docs": {},
                "y": {
                  "docs": {},
                  "|": {
                    "docs": {},
                    "n": {
                      "docs": {},
                      "u": {
                        "docs": {},
                        "l": {
                          "docs": {
                            "module-jskos-tools.ConceptScheme.html#isValidNotation": {
                              "ref": "module-jskos-tools.ConceptScheme.html#isValidNotation",
                              "tf": 33.33333333333333
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "n": {
          "docs": {},
          "o": {
            "docs": {},
            "t": {
              "docs": {},
              "a": {
                "docs": {},
                "t": {
                  "docs": {
                    "module-jskos-tools.ConceptScheme.html#isValidNotation": {
                      "ref": "module-jskos-tools.ConceptScheme.html#isValidNotation",
                      "tf": 40.47619047619047
                    },
                    "module-jskos-tools.ConceptScheme.html#notationFromUri": {
                      "ref": "module-jskos-tools.ConceptScheme.html#notationFromUri",
                      "tf": 3.8461538461538463
                    },
                    "module-jskos-tools.ConceptScheme.html#uriFromNotation": {
                      "ref": "module-jskos-tools.ConceptScheme.html#uriFromNotation",
                      "tf": 58.33333333333333
                    },
                    "module-jskos-tools.ConceptScheme.html#conceptFromNotation": {
                      "ref": "module-jskos-tools.ConceptScheme.html#conceptFromNotation",
                      "tf": 43.33333333333333
                    }
                  },
                  "i": {
                    "docs": {},
                    "o": {
                      "docs": {},
                      "n": {
                        "docs": {},
                        "p": {
                          "docs": {},
                          "a": {
                            "docs": {},
                            "t": {
                              "docs": {},
                              "t": {
                                "docs": {},
                                "e": {
                                  "docs": {},
                                  "r": {
                                    "docs": {},
                                    "n": {
                                      "docs": {
                                        "module-jskos-tools.ConceptScheme.html#isValidNotation": {
                                          "ref": "module-jskos-tools.ConceptScheme.html#isValidNotation",
                                          "tf": 7.142857142857142
                                        }
                                      }
                                    }
                                  }
                                }
                              }
                            }
                          }
                        },
                        "f": {
                          "docs": {},
                          "r": {
                            "docs": {},
                            "o": {
                              "docs": {},
                              "m": {
                                "docs": {},
                                "u": {
                                  "docs": {},
                                  "r": {
                                    "docs": {},
                                    "i": {
                                      "docs": {
                                        "module-jskos-tools.ConceptScheme.html#notationFromUri": {
                                          "ref": "module-jskos-tools.ConceptScheme.html#notationFromUri",
                                          "tf": 608.3333333333334
                                        }
                                      }
                                    }
                                  }
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "a": {
            "docs": {},
            "m": {
              "docs": {},
              "e": {
                "docs": {},
                "s": {
                  "docs": {},
                  "p": {
                    "docs": {},
                    "a": {
                      "docs": {},
                      "c": {
                        "docs": {
                          "module-jskos-tools.ConceptScheme.html#notationFromUri": {
                            "ref": "module-jskos-tools.ConceptScheme.html#notationFromUri",
                            "tf": 3.8461538461538463
                          },
                          "module-jskos-tools.ConceptScheme.html#uriFromNotation": {
                            "ref": "module-jskos-tools.ConceptScheme.html#uriFromNotation",
                            "tf": 4.166666666666666
                          },
                          "module-jskos-tools.ConceptScheme.html#conceptFromUri": {
                            "ref": "module-jskos-tools.ConceptScheme.html#conceptFromUri",
                            "tf": 4.166666666666666
                          },
                          "module-jskos-tools.ConceptScheme.html#conceptFromNotation": {
                            "ref": "module-jskos-tools.ConceptScheme.html#conceptFromNotation",
                            "tf": 5
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "v": {
          "docs": {},
          "a": {
            "docs": {},
            "l": {
              "docs": {},
              "i": {
                "docs": {},
                "d": {
                  "docs": {
                    "module-jskos-tools.ConceptScheme.html#isValidNotation": {
                      "ref": "module-jskos-tools.ConceptScheme.html#isValidNotation",
                      "tf": 7.142857142857142
                    },
                    "module-jskos-tools.ConceptScheme.html#uriFromNotation": {
                      "ref": "module-jskos-tools.ConceptScheme.html#uriFromNotation",
                      "tf": 4.166666666666666
                    },
                    "module-jskos-tools.ConceptScheme.html#conceptFromNotation": {
                      "ref": "module-jskos-tools.ConceptScheme.html#conceptFromNotation",
                      "tf": 5
                    }
                  }
                }
              }
            }
          }
        },
        "b": {
          "docs": {},
          "e": {
            "docs": {},
            "l": {
              "docs": {},
              "o": {
                "docs": {},
                "n": {
                  "docs": {},
                  "g": {
                    "docs": {
                      "module-jskos-tools.ConceptScheme.html#notationFromUri": {
                        "ref": "module-jskos-tools.ConceptScheme.html#notationFromUri",
                        "tf": 3.8461538461538463
                      },
                      "module-jskos-tools.ConceptScheme.html#conceptFromUri": {
                        "ref": "module-jskos-tools.ConceptScheme.html#conceptFromUri",
                        "tf": 4.166666666666666
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "u": {
          "docs": {},
          "r": {
            "docs": {},
            "i": {
              "docs": {
                "module-jskos-tools.ConceptScheme.html#notationFromUri": {
                  "ref": "module-jskos-tools.ConceptScheme.html#notationFromUri",
                  "tf": 37.179487179487175
                },
                "module-jskos-tools.ConceptScheme.html#uriFromNotation": {
                  "ref": "module-jskos-tools.ConceptScheme.html#uriFromNotation",
                  "tf": 4.166666666666666
                },
                "module-jskos-tools.ConceptScheme.html#conceptFromUri": {
                  "ref": "module-jskos-tools.ConceptScheme.html#conceptFromUri",
                  "tf": 37.49999999999999
                }
              },
              "p": {
                "docs": {},
                "a": {
                  "docs": {},
                  "t": {
                    "docs": {},
                    "t": {
                      "docs": {},
                      "e": {
                        "docs": {},
                        "r": {
                          "docs": {},
                          "n": {
                            "docs": {
                              "module-jskos-tools.ConceptScheme.html#notationFromUri": {
                                "ref": "module-jskos-tools.ConceptScheme.html#notationFromUri",
                                "tf": 3.8461538461538463
                              },
                              "module-jskos-tools.ConceptScheme.html#uriFromNotation": {
                                "ref": "module-jskos-tools.ConceptScheme.html#uriFromNotation",
                                "tf": 4.166666666666666
                              },
                              "module-jskos-tools.ConceptScheme.html#conceptFromUri": {
                                "ref": "module-jskos-tools.ConceptScheme.html#conceptFromUri",
                                "tf": 4.166666666666666
                              },
                              "module-jskos-tools.ConceptScheme.html#conceptFromNotation": {
                                "ref": "module-jskos-tools.ConceptScheme.html#conceptFromNotation",
                                "tf": 5
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              },
              "f": {
                "docs": {},
                "r": {
                  "docs": {},
                  "o": {
                    "docs": {},
                    "m": {
                      "docs": {},
                      "n": {
                        "docs": {},
                        "o": {
                          "docs": {},
                          "t": {
                            "docs": {
                              "module-jskos-tools.ConceptScheme.html#uriFromNotation": {
                                "ref": "module-jskos-tools.ConceptScheme.html#uriFromNotation",
                                "tf": 625
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "o": {
          "docs": {},
          "p": {
            "docs": {},
            "t": {
              "docs": {},
              "i": {
                "docs": {},
                "o": {
                  "docs": {},
                  "n": {
                    "docs": {
                      "module-jskos-tools.ConceptScheme.html#conceptFromUri": {
                        "ref": "module-jskos-tools.ConceptScheme.html#conceptFromUri",
                        "tf": 33.33333333333333
                      },
                      "module-jskos-tools.ConceptScheme.html#conceptFromNotation": {
                        "ref": "module-jskos-tools.ConceptScheme.html#conceptFromNotation",
                        "tf": 33.33333333333333
                      }
                    }
                  }
                }
              }
            }
          }
        }
      },
      "length": 107
    },
    "corpusTokens": [
      "array|nul",
      "belong",
      "check",
      "class",
      "concept",
      "conceptfromnot",
      "conceptfromuri",
      "conceptschem",
      "conceptscheme#conceptfromnot",
      "conceptscheme#conceptfromuri",
      "conceptscheme#isvalidnot",
      "conceptscheme#notationfromuri",
      "conceptscheme#urifromnot",
      "data",
      "defin",
      "document",
      "format",
      "function",
      "global",
      "index",
      "isvalidnot",
      "jsko",
      "list",
      "list:class",
      "list:modul",
      "local",
      "map",
      "modul",
      "module:jsko",
      "namespac",
      "notat",
      "notationfromuri",
      "notationpattern",
      "option",
      "readm",
      "requir",
      "return",
      "scheme",
      "string",
      "string|undefin",
      "success",
      "tool",
      "tools.conceptschem",
      "tools.conceptscheme#conceptfromnot",
      "tools.conceptscheme#conceptfromuri",
      "tools.conceptscheme#isvalidnot",
      "tools.conceptscheme#notationfromuri",
      "tools.conceptscheme#urifromnot",
      "uri",
      "urifromnot",
      "uripattern",
      "valid",
      "whether",
      "work"
    ],
    "pipeline": [
      "trimmer",
      "stopWordFilter",
      "stemmer"
    ]
  },
  "store": {
    "index.html": {
      "id": "index.html",
      "kind": "readme",
      "title": "jskos-tools",
      "longname": "index",
      "name": "jskos-tools",
      "tags": "index",
      "summary": "Tools for working with the JSKOS data format",
      "description": "",
      "body": ""
    },
    "global.html": {
      "id": "global.html",
      "kind": "global",
      "title": "Globals",
      "longname": "global",
      "name": "Globals",
      "tags": "global",
      "summary": "All documented globals.",
      "description": "",
      "body": ""
    },
    "list_class.html": {
      "id": "list_class.html",
      "kind": "list",
      "title": "Classes",
      "longname": "list:class",
      "name": "Classes",
      "tags": "list:class",
      "summary": "All documented classes.",
      "description": "",
      "body": ""
    },
    "list_module.html": {
      "id": "list_module.html",
      "kind": "list",
      "title": "Modules",
      "longname": "list:module",
      "name": "Modules",
      "tags": "list:module",
      "summary": "All documented modules.",
      "description": "",
      "body": ""
    },
    "module-jskos-tools.ConceptScheme.html": {
      "id": "module-jskos-tools.ConceptScheme.html",
      "kind": "class",
      "title": "jskos-tools.ConceptScheme",
      "longname": "module:jskos-tools.ConceptScheme",
      "name": "ConceptScheme",
      "tags": "module:jskos-tools.ConceptScheme ConceptScheme",
      "summary": "",
      "description": "A JSKOS Concept Scheme.",
      "body": ""
    },
    "module-jskos-tools.ConceptScheme.html#isValidNotation": {
      "id": "module-jskos-tools.ConceptScheme.html#isValidNotation",
      "kind": "function",
      "title": "isValidNotation( notation ) → {array|null}",
      "longname": "module:jskos-tools.ConceptScheme#isValidNotation",
      "name": "isValidNotation",
      "tags": "module:jskos-tools.ConceptScheme#isValidNotation ConceptScheme#isValidNotation isValidNotation",
      "summary": "",
      "description": "Check whether a string is a valid notation as defined by notationPattern."
    },
    "module-jskos-tools.ConceptScheme.html#notationFromUri": {
      "id": "module-jskos-tools.ConceptScheme.html#notationFromUri",
      "kind": "function",
      "title": "notationFromUri( uri ) → {string|undefined}",
      "longname": "module:jskos-tools.ConceptScheme#notationFromUri",
      "name": "notationFromUri",
      "tags": "module:jskos-tools.ConceptScheme#notationFromUri ConceptScheme#notationFromUri notationFromUri",
      "summary": "",
      "description": "Check whether URI belongs to the scheme, return local notation on success. Requires scheme to have uriPattern or namespace."
    },
    "module-jskos-tools.ConceptScheme.html#uriFromNotation": {
      "id": "module-jskos-tools.ConceptScheme.html#uriFromNotation",
      "kind": "function",
      "title": "uriFromNotation( notation )",
      "longname": "module:jskos-tools.ConceptScheme#uriFromNotation",
      "name": "uriFromNotation",
      "tags": "module:jskos-tools.ConceptScheme#uriFromNotation ConceptScheme#uriFromNotation uriFromNotation",
      "summary": "",
      "description": "Map local notation to URI. Does not check whether notation is valid! Requires scheme to have uriPattern or namespace."
    },
    "module-jskos-tools.ConceptScheme.html#conceptFromUri": {
      "id": "module-jskos-tools.ConceptScheme.html#conceptFromUri",
      "kind": "function",
      "title": "conceptFromUri( uri [, options ] )",
      "longname": "module:jskos-tools.ConceptScheme#conceptFromUri",
      "name": "conceptFromUri",
      "tags": "module:jskos-tools.ConceptScheme#conceptFromUri ConceptScheme#conceptFromUri conceptFromUri",
      "summary": "",
      "description": "Check whether URI belongs to the scheme, return concept on success. Requires scheme to have uriPattern or namespace."
    },
    "module-jskos-tools.ConceptScheme.html#conceptFromNotation": {
      "id": "module-jskos-tools.ConceptScheme.html#conceptFromNotation",
      "kind": "function",
      "title": "conceptFromNotation( notation [, options ] )",
      "longname": "module:jskos-tools.ConceptScheme#conceptFromNotation",
      "name": "conceptFromNotation",
      "tags": "module:jskos-tools.ConceptScheme#conceptFromNotation ConceptScheme#conceptFromNotation conceptFromNotation",
      "summary": "",
      "description": "Map local notation to concept, if notation is valid. Requires scheme to have uriPattern or namespace."
    },
    "module-jskos-tools.html": {
      "id": "module-jskos-tools.html",
      "kind": "module",
      "title": "jskos-tools",
      "longname": "module:jskos-tools",
      "name": "jskos-tools",
      "tags": "module:jskos-tools",
      "summary": "",
      "description": "JSKOS Tools.",
      "body": ""
    }
  }
};